...
public class TicTacToe extends Canvas {
    ...
    /**
     * This will create a 2D integer array of dimensions of any size
     *
     * @param rows    int
     * @param columns int
     * @return returns a 2D integer array of dimensions of any size
     */
    ...
    /**
     * checks the size of rows in the 2D integer array
     *
     * @param board 2D integer array
     * @return returns the size of the first dimension of the array which is the rows.
     */
    ...
    /**
     * checks the size of columns in the 2D integer array
     *
     * @param board 2D integer array
     * @return returns the size of the second dimension of the array which is the columns.
     */
    ...
    /**
     * this will check to see if the location on the array is empty or not.
     *
     * @param board  2D integer array
     * @param row    int
     * @param column int
     * @return will return True if space is empty, and False if space is not empty.
     */
    ...
    /**
     * this function will start the game, and then it will assign a piece of either X or O on an empty location
     *
     * @param board  2D integer array
     * @param row    int
     * @param column int
    ...
    /**
     * this function will check if the board is full,and it will return a value
     * of either true or false depending on whether the board is full or empty.
     *
     * @param board 2D integer array
     * @return it will return true if the board is full, false otherwise.
     */
    public static boolean full(int[][] board) {
        for (int[] row : board) { //accessing the row on that board
            for (int column : row) { //accessing a column on that row specifically
                if (column == EMPTY) { //checks to see if that element/position is empty
                    return false;
                }
            }
        }
    ...
    /**
     * this function checks if a row in the board is filled with 3 of the same type of pieces consecutively,
     * so either all 3 X's or all 3 O's. A true or false value is returned whether the row is empty or not.
     *
     * @param board 2D integer array
     * @param row   int
     * @param piece int
     * @return it will return true if the row has either 3 X's or 3 O's consecutively, false otherwise.
     */
    public static boolean winInRow(int[][] board, int row, int piece) {
        int row_count = 0; //counter is set to zero
        for (int column : board[row]) { //iterates through each row in the array
            if (column == piece) { //if the column in that row is equal to the piece (X or O)
                row_count++; //if it is equal to the piece, it moves to the next element of the row
                if (row_count == 3) { //then it checks if it has 3 consecutive wins
                    return true;
                }
            } else {
                row_count = 0;
    ...
    /**
     * this function checks if a column in the board is filled with 3 of the same type of pieces consecutively,
     * so either all 3 X's or al 3 O's. A true or false value is returned whether the column is empty or not.
     *
     * @param board  2D integer array
     * @param column int
     * @param piece  int
     * @return it will return tru if the column has either 3 X's or 3 O's consecutively, false otherwise.
     */
    public static boolean winInColumn(int[][] board, int column, int piece) {
        int column_count = 0; // counter is set to zero
        for (int row : board[column]) { //iterates through each column in the array
            if (row == piece) { //if the row in that column is equal to the piece (X or O)
                column_count++; // if it is equal to the piece, then it moves onto the next element
                if (column_count == 3) { //then checks if it has 3 consecutive wins
                    return true;
                }
            } else {
                column_count = 0;
    ...
    /**
     * this function checks if the element it is on will give the user a win or not, if it does then it will give
     * the user a hint, if it will not let the user win on that element, it goes and checks for the other ones.
     * It will return the hint depending on the location it is found at.
     *
     * @param board 2D integer array
     * @param piece int
     * @return returns the hint given to the user, which is the location at which the hint was given.
     */
    public static int[] hint(int[][] board, int piece) {
        int row = rowsIn(board);
        int column = columnsIn(board);

        for (row = 0; row < row; row++) { //using a for loop so that the condition makes is work for any size array
            for (column = 0; column < column; column++) { //using a for loop so that the condition makes is work for any size array
                if (canPlay(board, row, column) == true) {
    ...
    private static void run() {
        ...
        while (!isGameOver(board)) {
            ...
            if (human == player) {
                ...
                //If AI then gets the play and complete it
